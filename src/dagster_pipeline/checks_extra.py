...
@asset_check(
    asset="filtered_articles",
    description="No duplicate pubmed_id values allowed in pubmed_article",
)
...
@asset_check(
    asset="filtered_articles",
    description="No null titles allowed in pubmed_article",
)
...
@asset_check(
    asset="llm_extraction",
    description="Extraction confidence must be between 0 and 1 for all rows",
)
def extraction_confidence_range_check(database: DatabaseResource) -> AssetCheckResult:
    with database.get_session() as session:
        out_of_range = session.query(LlmExtraction).filter(
            (LlmExtraction.extraction_confidence < 0)
            | (LlmExtraction.extraction_confidence > 1)
        ).count()
    return AssetCheckResult(
